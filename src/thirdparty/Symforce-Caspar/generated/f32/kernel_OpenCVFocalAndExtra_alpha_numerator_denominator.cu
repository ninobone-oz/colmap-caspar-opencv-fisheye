...
__global__ void __launch_bounds__(1024, 1)
    OpenCVFocalAndExtraAlphaNumeratorDenominatorKernel(
        ...
        float* const OpenCVFocalAndExtra_total_ac,
        size_t problem_size) {
  ...
  if (global_thread_idx < problem_size) {
    ReadIdx2<1024, float, float, float2>(
        OpenCVFocalAndExtra_p_kp1,
        4 * OpenCVFocalAndExtra_p_kp1_num_alloc,
        global_thread_idx,
        r0,
        r1);
    ...
    ReadIdx4<1024, float, float, float4>(
        OpenCVFocalAndExtra_p_kp1,
        0 * OpenCVFocalAndExtra_p_kp1_num_alloc,
        global_thread_idx,
        r3,
        r4,
        r5,
  ...
  SumStore<float>(OpenCVFocalAndExtra_total_ag_local,
                  (float*)inout_shared,
                  0,
  ...
  SumStore<float>(OpenCVFocalAndExtra_total_ac_local,
                  (float*)inout_shared,
                  0,
                  global_thread_idx < problem_size,
                  r7);
  SumFlushFinal<float>(
      OpenCVFocalAndExtra_total_ag_local, OpenCVFocalAndExtra_total_ag, 1);
  SumFlushFinal<float>(
      OpenCVFocalAndExtra_total_ac_local, OpenCVFocalAndExtra_total_ac, 1);
}
...
void OpenCVFocalAndExtraAlphaNumeratorDenominator(
  ...
  OpenCVFocalAndExtraAlphaNumeratorDenominatorKernel<<<n_blocks, 1024>>>(
      OpenCVFocalAndExtra_p_kp1,
      OpenCVFocalAndExtra_p_kp1_num_alloc,
      OpenCVFocalAndExtra_r_k,
      OpenCVFocalAndExtra_r_k_num_alloc,
      OpenCVFocalAndExtra_w,
      OpenCVFocalAndExtra_w_num_alloc,
      OpenCVFocalAndExtra_total_ag,
      OpenCVFocalAndExtra_total_ac,
      problem_size);
}
