...
__global__ void __launch_bounds__(1024, 1)
    OpencvJtjnjtrDirectKernel(float* pose_njtr,
  ...
  if (global_thread_idx < problem_size) {
    ReadIdx2<1024, float, float, float2>(
        calib_jac, 8 * calib_jac_num_alloc, global_thread_idx, r8, r9);
    r7 = fmaf(r5, r9, r7);
  ...
  if (global_thread_idx < problem_size) {
    ReadIdx2<1024, float, float, float2>(
        point_jac, 4 * point_jac_num_alloc, global_thread_idx, r24, r25);
    ...
    ReadIdx4<1024, float, float, float4>(
        pose_jac, 4 * pose_jac_num_alloc, global_thread_idx, r17, r16, r4, r6);
...
void OpencvJtjnjtrDirect(float* pose_njtr,
                         unsigned int pose_njtr_num_alloc,
                         SharedIndex* pose_njtr_indices,
                         float* pose_jac,
                         unsigned int pose_jac_num_alloc,
  ...
  OpencvJtjnjtrDirectKernel<<<n_blocks, 1024>>>(pose_njtr,
                                                pose_njtr_num_alloc,
                                                pose_njtr_indices,
                                                pose_jac,
                                                pose_jac_num_alloc,
