...
__global__ void __launch_bounds__(1024, 1)
    OpenCVFocalAndExtraAlphaDenominatorOrBetaNumeratorKernel(
        ...
        float* const OpenCVFocalAndExtra_out,
        size_t problem_size) {
  ...
  SumStore<float>(OpenCVFocalAndExtra_out_local,
                  (float*)inout_shared,
                  0,
                  global_thread_idx < problem_size,
                  r6);
  SumFlushFinal<float>(
      OpenCVFocalAndExtra_out_local, OpenCVFocalAndExtra_out, 1);
}
