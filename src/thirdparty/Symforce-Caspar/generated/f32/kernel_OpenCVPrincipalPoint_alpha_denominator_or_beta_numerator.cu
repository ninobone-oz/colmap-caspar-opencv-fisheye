...
__global__ void __launch_bounds__(1024, 1)
    OpenCVPrincipalPointAlphaDenominatorOrBetaNumeratorKernel(
        ...
        float* const OpenCVPrincipalPoint_out,
        size_t problem_size) {
  ...
  SumStore<float>(OpenCVPrincipalPoint_out_local,
                  (float*)inout_shared,
                  0,
                  global_thread_idx < problem_size,
                  r2);
  SumFlushFinal<float>(
      OpenCVPrincipalPoint_out_local, OpenCVPrincipalPoint_out, 1);
}
