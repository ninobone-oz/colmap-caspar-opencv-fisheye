...
__global__ void __launch_bounds__(1024, 1) OpenCVPosePredDecreaseTimesTwoKernel(
    float* OpenCVPose_step,
    unsigned int OpenCVPose_step_num_alloc,
    float* OpenCVPose_precond_diag,
    unsigned int OpenCVPose_precond_diag_num_alloc,
    const float* const diag,
    float* OpenCVPose_njtr,
    unsigned int OpenCVPose_njtr_num_alloc,
    float* const out_OpenCVPose_pred_dec,
    size_t problem_size) {
  ...
  SumStore<float>(out_OpenCVPose_pred_dec_local,
                  (float*)inout_shared,
                  0,
                  global_thread_idx < problem_size,
                  r4);
  SumFlushFinal<float>(
      out_OpenCVPose_pred_dec_local, out_OpenCVPose_pred_dec, 1);
}
...
void OpenCVPosePredDecreaseTimesTwo(
  ...
  OpenCVPosePredDecreaseTimesTwoKernel<<<n_blocks, 1024>>>(
      OpenCVPose_step,
      OpenCVPose_step_num_alloc,
      OpenCVPose_precond_diag,
      OpenCVPose_precond_diag_num_alloc,
      diag,
      OpenCVPose_njtr,
      OpenCVPose_njtr_num_alloc,
      out_OpenCVPose_pred_dec,
      problem_size);
}
