...
__global__ void __launch_bounds__(1024, 1) OpenCVFocalAndExtraRetractKernel(
    float* OpenCVFocalAndExtra,
    unsigned int OpenCVFocalAndExtra_num_alloc,
    float* delta,
  ...
  if (global_thread_idx < problem_size) {
    ...
    WriteIdx4<1024, float, float, float4>(
        ...
        0 * out_OpenCVFocalAndExtra_retracted_num_alloc,
        global_thread_idx,
        r4,
        r5,
        r6,
    ...
    ReadIdx2<1024, float, float, float2>(
        delta, 4 * delta_num_alloc, global_thread_idx, r5, r4);
    r5 = r7 + r5;
    r4 = r6 + r4;
    WriteIdx2<1024, float, float, float2>(
        out_OpenCVFocalAndExtra_retracted,
        4 * out_OpenCVFocalAndExtra_retracted_num_alloc,
        global_thread_idx,
        r5,
        r4);
  };
...
void OpenCVFocalAndExtraRetract(
  ...
  OpenCVFocalAndExtraRetractKernel<<<n_blocks, 1024>>>(
      OpenCVFocalAndExtra,
      OpenCVFocalAndExtra_num_alloc,
      delta,
      delta_num_alloc,
      out_OpenCVFocalAndExtra_retracted,
      out_OpenCVFocalAndExtra_retracted_num_alloc,
      problem_size);
}
