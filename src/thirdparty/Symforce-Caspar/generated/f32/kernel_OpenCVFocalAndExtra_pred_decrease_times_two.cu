...
__global__ void __launch_bounds__(1024, 1)
    OpenCVFocalAndExtraPredDecreaseTimesTwoKernel(
        ...
        float* OpenCVFocalAndExtra_njtr,
        unsigned int OpenCVFocalAndExtra_njtr_num_alloc,
        float* const out_OpenCVFocalAndExtra_pred_dec,
        size_t problem_size) {
  ...
  SumStore<float>(out_OpenCVFocalAndExtra_pred_dec_local,
                  (float*)inout_shared,
                  0,
                  global_thread_idx < problem_size,
                  r5);
  SumFlushFinal<float>(out_OpenCVFocalAndExtra_pred_dec_local,
                       out_OpenCVFocalAndExtra_pred_dec,
                       1);
}
...
void OpenCVFocalAndExtraPredDecreaseTimesTwo(
  ...
  OpenCVFocalAndExtraPredDecreaseTimesTwoKernel<<<n_blocks, 1024>>>(
      OpenCVFocalAndExtra_step,
      OpenCVFocalAndExtra_step_num_alloc,
      OpenCVFocalAndExtra_precond_diag,
      OpenCVFocalAndExtra_precond_diag_num_alloc,
      diag,
      OpenCVFocalAndExtra_njtr,
      OpenCVFocalAndExtra_njtr_num_alloc,
      out_OpenCVFocalAndExtra_pred_dec,
      problem_size);
}
